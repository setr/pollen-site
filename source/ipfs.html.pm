...
The reason you can't download a file with no seeders on it, using the Bittorrent protocol, is because the DHT only stores metadata for ◊emph{single files}. 

IPFS removes this limitation, as the IPFS DHT is global. When it requests a block from the network, ◊emph{it does not ask where that block came from}. In other words, you can gather blocks from ◊emph{people who never saw the file}. 
...
Also, if I understand it correctly, what the block-based structure ◊emph{should} mean is something like this:
...
=>the only pieces that would actually be ◊emph{dependent} on the original png existing on the network are those wholly unique to that png
